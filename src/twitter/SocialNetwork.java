...
public class SocialNetwork {
	...
	public static Map<String, Set<String>> guessFollowsGraph(List<Tweet> tweets) {
	    ...
	    for (Tweet tweet : tweets) {
	        ...
	        while (matcher.find()) {
	            ...
	            // Add mention as a follower relationship
	...
	public static List<String> influencers(Map<String, Set<String>> followsGraph) {
	    ...
	    // Create a list of influencers (users with at least one follower)
	    List<String> influencers = new ArrayList<>();
	    for (String user : followsGraph.keySet()) {
	        if (followerCount.getOrDefault(user, 0) > 0) {
	            influencers.add(user);
	        }
	    }

	    // Sort influencers by follower count in descending order
	    influencers.sort((a, b) -> Integer.compare(followerCount.getOrDefault(b, 0), followerCount.getOrDefault(a, 0)));
